...
int saveTmpToArr(struct MainArray *mainarr)
{
    ...
    mainarr->length = mainarr->length + pairsNum;
    ...
    rewind(tmpFile);
    char *tmpOpStr = calloc(fileLength + 1, sizeof(char));
    fgets(str, 100, tmpFile);
    ...
    while (fgets(str, 100, tmpFile) != NULL)
    {
        if (isdigit(str[0]))
        {
            ...
            {
                mainarr->oparr[i]->op[j] = calloc(strlen(tmpOpStr) + 1, sizeof(char));
