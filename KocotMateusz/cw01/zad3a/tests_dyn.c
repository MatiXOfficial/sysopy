...
{
    void *handle = dlopen("./libdifflib.so", RTLD_LAZY);
	if(!handle)
    {
        printf("Error finding library!");
        return 1;
	}

    struct MainArray* (*createMainArray)() = dlsym(handle, "createMainArray");
    void (*saveDiffToTmp)() = dlsym(handle, "saveDiffToTmp");
    int (*saveTmpToArr)() = dlsym(handle, "saveTmpToArr");
    void (*removeOpBlock)() = dlsym(handle, "removeOpBlock");
    void (*removeOp)() = dlsym(handle, "removeOp");
    void (*freeAll)() = dlsym(handle, "freeAll");
