...
int main(int argc, char *argv[])
{
    ...
    {
        fd_client = socket(AF_UNIX, SOCK_DGRAM, 0);
        if (fd_client == -1)
            error("socket error");
        ...
        if (bind(fd_client, (struct sockaddr *)&addr_client, sizeof(addr_client)) == -1)
            error("bind error");

        struct sockaddr_un addr_clientConnect = {.sun_family = AF_UNIX};
        strcpy(addr_clientConnect.sun_path, argv[3]);

        if (connect(fd_client, (struct sockaddr *)&addr_clientConnect, sizeof(addr_clientConnect)) == -1)
            error("connect error");
    }
    else
    {
